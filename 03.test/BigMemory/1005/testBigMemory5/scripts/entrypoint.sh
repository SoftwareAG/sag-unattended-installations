...
onInterrupt(){
	...
	${SUIF_INSTALL_DIR}/Terracotta/tools/management-console/bin/stop-tmc.sh
    pushd . >/dev/null
    cd "${SUIF_INSTALL_DIR}/Terracotta/server/wrapper/bin/"
    ./shutdown.sh
    popd >/dev/null
...
controlledExec "${SUIF_INSTALL_DIR}/Terracotta/tools/management-console/bin/start-tmc.sh" "Start TMC" &
