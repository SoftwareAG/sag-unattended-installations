...
:: ------------------------------
:: Ensure OS level settings are ok
:: ------------------------------
powershell.exe -NonInteractive -ExecutionPolicy Unrestricted -Command "& {. '.\scripts\suif_ps_functions.ps1'; ensureOSSettings $LastExitCode}"
if "%errorlevel%" NEQ "0" (
    echo Unable to verify OS settings ... exiting
    goto end
)
...
:: Upload License key
:: ------------------------------
powershell.exe -NonInteractive -ExecutionPolicy Unrestricted -Command "& {. '.\scripts\suif_ps_functions.ps1'; uploadFileToVM -suif_source_handle 'H_API_GW_LICENSE_FILE' -suif_target_handle 'SUIF_SETUP_TEMPLATE_YAI_LICENSE_FILE' $LastExitCode}"
if "%errorlevel%" NEQ "0" (
    echo Unable to provision the VM ... exiting
    goto end
)
...
:: ------------------------------
powershell.exe -NonInteractive -ExecutionPolicy Unrestricted -Command "& {. '.\scripts\suif_ps_functions.ps1'; createInboundFWRule -az_nsg_rule_name 'SUIF_APIGW_IS_HTTP' -az_nsg_rule_prio 1500 -az_nsg_port 5555 $LastExitCode}"
if "%errorlevel%" NEQ "0" (
    echo Unable to create inbound security rule ... exiting
    goto end
)
powershell.exe -NonInteractive -ExecutionPolicy Unrestricted -Command "& {. '.\scripts\suif_ps_functions.ps1'; createInboundFWRule -az_nsg_rule_name 'SUIF_APIGW_UI_HTTP' -az_nsg_rule_prio 1501 -az_nsg_port 9072 $LastExitCode}"
if "%errorlevel%" NEQ "0" (
    echo Unable to create inbound security rule ... exiting
    goto end
)
powershell.exe -NonInteractive -ExecutionPolicy Unrestricted -Command "& {. '.\scripts\suif_ps_functions.ps1'; createInboundFWRule -az_nsg_rule_name 'SUIF_APIGW_UI_HTTPS' -az_nsg_rule_prio 1502 -az_nsg_port 9073 $LastExitCode}"
if "%errorlevel%" NEQ "0" (
    echo Unable to create inbound security rule ... exiting
    goto end
)
