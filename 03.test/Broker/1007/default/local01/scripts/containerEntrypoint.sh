...
SUIF_HOME=${SUIF_HOME:-/mnt/SUIF_HOME}
...
if [ ! -d "${SUIF_INSTALL_INSTALL_DIR}/IntegrationServer" ]; then
    ...
    applySetupTemplate "Broker/1007/default" || tail -f /dev/null # exit 6
fi

onInterrupt(){

    ## logI "Interrupted, shutting down Broker Server ..."

    logI "Interrupted, shutting down Broker Monitor ..."
...
logI "Waiting for Broker Monitor to finish..."

tail -f /dev/null

logI "Broker Monitor was shut down"