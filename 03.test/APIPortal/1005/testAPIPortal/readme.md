...
The purpose of this test is to validate the local setup scripts for API Portal version 1005
...
6. Open a browser to [localhost API Portal UI](http://localhost:18101) (or change port if you changed the port prefix)


## Reusing the scripts for other environments

This test is given as an example of on-premise installation. In a classical environment, the following steps are needed to obtain an API Portal node
...
4. Prepare your own scripts mimicking the scripts in this test harness
   ...
   2. containerEntrypoint.sh must be adapted to your environment specifics, follow the provided sequence
      1. to install, first set the necessary env variables as shown, then apply the template "APIPortal/1005/default"
