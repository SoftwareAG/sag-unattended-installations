...
. "${SUIF_HOME}/installationScripts/commonFunctions.sh" || exit 4
. "${SUIF_HOME}/installationScripts/setupFunctions.sh" || exit 5
...
if [ ! -d "${SUIF_INSTALL_InstallDir}/IntegrationServer" ]; then
    ...
    applySetupTemplate "templates/setup" || exit 6
fi

onInterrupt(){
	echo "Interrupted! Shutting down MFT Gateway"
    pushd . >/dev/null
...
afterStartConfig(){
    ...
    applyPostSetupTemplate templates/setup
...
echo "Starting up MFT Gateway server"
