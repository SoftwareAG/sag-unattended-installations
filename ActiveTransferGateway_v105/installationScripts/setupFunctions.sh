...
# $1 - Setup template directory, relative to <repo_home>/templates/setup
applySetupTemplate(){
    logI "Applying Setup Template ${1}"
    huntForSuifFile "templates/setup/${1}" "template.wmscript" || return 1
    huntForSuifFile "templates/setup/${1}" "setEnvDefaults.sh" || return 2
    huntForSuifFile "templates/setup/${1}" "checkPrerequisites.sh" || return 4
    logI "Sourcing variable values for template ${1} ..."
    . "${SUIF_CACHE_HOME}/templates/setup/setEnvDefaults.sh"
    logI "Checking installation prerequisites for template ${1} ..."
    chmod u+x "${SUIF_CACHE_HOME}/templates/setup/checkPrerequisites.sh" > /dev/null
    "${SUIF_CACHE_HOME}/templates/setup/checkPrerequisites.sh" || return 5
    logI "Setting up products and fixes for template ${1} ..."
    setupProductsAndFixes \
        "${SUIF_INSTALL_INSTALLER_BIN}" \
        "${SUIF_CACHE_HOME}/templates/setup/template.wmscript" \
