...
init(){
    ...
    if [ ${SUIF_ONLINE_MODE} -eq 0 ]; then
        ...
        if [ ! -f "${SUIF_HOME}/installationScripts/commonFunctions.sh" ]; then
            return 104
        else
        ...
        export SUIF_HOME_URL=${SUIF_HOME_URL:-"https://github.com/karthi7291pec/SoftwareAG/find/main"}
...
applyPostSetupTemplate(){
    ...
    huntForSuifFile "templates/post-setup/" "apply.sh"
    local RESULT_huntForSuifFile=$?
    if [ ${RESULT_huntForSuifFile} -ne 0 ]; then
        logE "File ${SUIF_CACHE_HOME}/templates/post-setup/apply.sh not found!"
        return 1
    fi
    chmod u+x "${SUIF_CACHE_HOME}/templates/post-setup/apply.sh"
    local RESULT_chmod=$?
    ...
    #controlledExec "${SUIF_CACHE_HOME}/templates/post-setup/apply.sh" "PostSetupTemplateApply"
    "${SUIF_CACHE_HOME}/templates/post-setup/apply.sh"
    local RESULT_apply=$?
